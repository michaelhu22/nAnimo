...
progLayout = fruchterman_reingold_layout_edit(progNetwork, iterations = 50, seed = 1, pretendIterations=1000, stop = 200)
gmpLayout = fruchterman_reingold_layout_edit(gmpNetwork, iterations = 50, seed = 1, pretendIterations=1000, stop = 200)
# end of network import
...
print(len(networks[0].edges()))
print(len(networks[1].edges()))
print(len(networks[2].edges()))
print(len(networks[9].edges()))


print(convertNode(networks[9]))
...
origFrames = [formatPos(progNetwork.nodes(data = True), progLayout, 1500, True),formatPos(gmpNetwork.nodes(data = True),gmpLayout, 1500, True)]
...
edgeList = [formatEdge(convertEdge(progNetwork)), formatEdge(convertEdge(gmpNetwork))]

formatOrigFrames = origFrames
...
timePoints = {0.0: formatOrigFrames[0], 0.9: formatOrigFrames[1]}
...
app.clientside_callback(
    ...
    function (value, max, frameData, edgeData, timePoints){
        ...
        for (let timePoint = 0; timePoint < timePointsKeys.length; timePoint++){
            if (value == timePointsKeys[timePoint]){
                return 'hi'
            }
        }
...
#         function(n_clicks){
#             if(n_clicks>0){
#                 return origFrames[1].concat(edgeList[1])
#             }
#         }
#     """,
